...
const Input: React.FC<Props> = (props) => {
  const { id, label, value, multiline = false, type = 'text', placeholder, rows, onChange } = props;

  const control = multiline ? (
    <TxtArea id={id} rows={rows} value={value} onChange={(e) => onChange(id, e.target.value)} />
  ) : (
    <TxtInput
      type={type}
      id={id}
